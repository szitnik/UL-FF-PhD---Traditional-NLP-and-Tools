...
class ExtractContentHelper():
    ...
    def is_inline(node: aw.Node):
        ...
        return ((node.get_ancestor(aw.NodeType.PARAGRAPH) is not None or node.get_ancestor(
            aw.NodeType.TABLE) is not None) and
    ...
    def process_marker(clone_node: aw.Node, nodes, node: aw.Node, block_level_ancestor: aw.Node,
        ...
        for i in range(len(node_branch) - 1, -1, -1):
            ...
            current_clone_node = current_clone_node.as_composite_node().get_child_nodes(aw.NodeType.ANY, False)[
                node_index]

            ExtractContentHelper.remove_nodes_outside_of_range(current_clone_node, is_inclusive or (i > 0),
                                                               is_start_marker)
